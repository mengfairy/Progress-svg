...
(function(global){
    var tool = {
        init: function(elem,option,funName){
        ...
        attr: function(elem,option){
            this.init(elem,option,'setAttribute');
        },
        attrNS: function(elem,option){
            this.init(elem,option,'setAttributeNS');
        },
        css: function(elem,option){
    ...
    function ProgressCreate(options){
        ...
        // baseAttr & baseStyle
        var startY = -Rr,circleStartY= 2*Rr,circleEndY = -2*Rr,dasharray= `${c}px 1000px`,dashoffset = 0,fullPercent = c;
        if(type === 'dashboard'){
